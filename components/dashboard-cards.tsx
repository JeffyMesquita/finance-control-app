...
export function DashboardCards() {
  ...
  // IDs dos cards que têm funcionalidade de hide/show
  ...
  const allCardsVisible = cardsWithValues.every(
    (cardId) => visibleCards[cardId]
  );

  // Função para alternar todos os cards
  const toggleAllCards = () => {
    const newVisibility = !allCardsVisible;
    const newVisibleCards: Record<string, boolean> = {};

    cardsWithValues.forEach((cardId) => {
      newVisibleCards[cardId] = newVisibility;
    });
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Botão para mostrar/esconder todos */}
      ...
      {/* Grid dos cards */}
      <div className="lg:grid lg:grid-cols-2 xl:grid-cols-4 flex flex-col gap-4">
        {/* Card Saldo Total */}
